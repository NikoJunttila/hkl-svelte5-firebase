...
async function createCard(){
 ...
 createSomething("cards",cardObj,id) 
 notifications.success("Kortti luotu",5000)
 urlForImg = ""
		setTimeout(async () => {
			cards = await fetchDocuments('cards');
		}, 1000);
}	
  /**
...
//@ts-ignore
$effect(async () => {
		const response = await fetchDocuments('cards');
	...
	<div class="flex flex-col gap-2 justify-center items-center">
		<h2 class="card-title">Lisää uusi kortti kuva</h2>
